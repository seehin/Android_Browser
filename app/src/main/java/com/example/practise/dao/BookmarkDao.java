...
public interface BookmarkDao {
    ...
    LiveData<List<BookmarkBean>> getUpperBookmark(int id);

    //根据id拿到子书签中的文件夹或者网址
    @Query("SELECT * FROM bookmark where upper = :id and isFolder in (:isFolders)")
    List<BookmarkBean> getBookmarkOfSubfolder(int id, int[] isFolders);

    //获取所有文件夹
    @Query("SELECT * FROM bookmark where isFolder = 1 and id not in (:id) and id <> :upper")
    List<BookmarkBean> getAllFolder(List<Integer> id, int upper);
    ...
    void insertBookmark(BookmarkBean... bookmarkBeans);
    ...
    //修改书签
    @Update()
    void alterBookmark(BookmarkBean... bookmarkBeans);

    // 删除数据
    @Delete()
    void deleteBookmarks(BookmarkBean... bookmarkBeans);

    //根据url来删除网址书签
    @Query("select * from bookmark where burl = :url")
    BookmarkBean getBookmarkBasisUrl(String url);
}
