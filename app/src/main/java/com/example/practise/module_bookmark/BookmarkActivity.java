...
import com.example.practise.databinding.ActivityBookmarkBinding;
import com.example.practise.databinding.AlterBookmarkDialogBinding;
import com.example.practise.databinding.BookmarkCheckedMoveToFolderBinding;
import com.example.practise.databinding.BookmarkDeleteAffirmDialogBinding;
import com.example.practise.databinding.BookmarkNewFolderDialogBinding;
...
public class BookmarkActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter.setItemClick((View view, BookmarkBean bookmarkBean)-> {
            ...
            if (imageOrText == R.id.one_bookmark_text){
                if(!isEdit){
                    if (bookmarkBean.getIsFolder() == 1) {//1代表点击的是文件夹
                        ...
                        //这里到时候实现点击跳转网页
                        Intent intent = new Intent(BookmarkActivity.this, NavigationActivity.class);
        ...
        LiveData<List<BookmarkBean>> listLiveData;
        //判断是否在进行搜索操作
        ...
        listLiveData.observe(this, (List<BookmarkBean> bookmarkList)-> {
    ...
    public void checkedDelete(List<BookmarkBean> checkedItems){
        if (checkedItems.size() == 0){
            Toast.makeText(BookmarkActivity.this, "无内容可删除", Toast.LENGTH_SHORT).show();
            return;
        }
        List<BookmarkBean> needDelete = new ArrayList<>(checkedItems);
    ...
    //弹窗确认删除
    public void deleteAffirm(Context context, List<BookmarkBean> needDelete, int num, int upper){
    ...
    //点击书签图标弹出菜单
    public void loadPopupMenu(View view, BookmarkBean bookmarkBean){
        //创建弹出式菜单对象（最低版本11）
        ...
        popup.setOnMenuItemClickListener((MenuItem item)-> {
            ...
            if (itemId == R.id.action_alter){
                ...
                List<BookmarkBean> checkedItems = new ArrayList<>();
    ...
    //弹出修改的操作框
    public void loadAlterDialog(Context context, BookmarkBean bookmarkBean){
    ...
    public void handleAlterInput(String alterNameString, String alterUrlString, BookmarkBean bookmarkBean, AlertDialog dialog){
    ...
    //修改书签的操作(名称和网址)
    public void alterBookmark(BookmarkBean bookmarkBean, String name, String url){
...
//            String regex = "(ht|f)tp(s?)\\:\\/\\/[0-9a-zA-Z]([-.\\w]*[0-9a-zA-Z])*(:(0-9)*)*(\\/?)([a-zA-Z0-9\\-\\.\\?\\,\\'\\/\\\\&%\\+\\$#_=]*)?";
    ...
    public void addNewFolder(String name, AlertDialog dialog){
        ...
        if(isexit == 1){
            ...
            bookmarkViewModel.insertBookmark(new BookmarkBean(name,"","",0,1, now_id, maxsort == null?0:maxsort+1));
    ...
    public void moveBookmark(Context context){
        ...
        //以下部分是对数据进行处理
        List<BookmarkBean> checkedItems = adapter.getCheckedItems();//获取所要移动的书签
        List<Integer> checkedFolderIds = new ArrayList<>();//把所有已选项中的文件夹的id存起来
        List<BookmarkBean> temp = new ArrayList<>(checkedItems);
        int[] isFolders = {1};
        ...
        List<BookmarkBean> allFolder = bookmarkViewModel.getAllFolder(checkedFolderIds, now_id);
        if (allFolder.size() == 0){
        ...
        MoveToFolderAdapter moveToFolderAdapter = new MoveToFolderAdapter(allFolder, (View view, BookmarkBean bookmarkBean)-> {
        ...
        dialogBinding.setMovetofolderclick((View v)-> {
            ...
            if (cancelOrAffirm == R.id.move_cancel){
                ...
                if (movetoId[0] == -1){
                    ...
                    for(BookmarkBean bookmarkBean:checkedItems){
